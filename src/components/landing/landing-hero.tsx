      ...
      >
        <div className="flex flex-col px-6 py-4 gap-4 text-sm font-medium">
          ...
          <Link
            ...
            className="text-gray-600 font-medium text-center py-1"
          >
            Entrar
          </Link>
          <Link
            href="/cadastro"
            className="bg-gradient-to-r from-[#1B2A6B] to-[#2DC6C6] text-white text-center py-3 rounded-full font-semibold"
          >
            Teste grátis — 3 dias grátis
          </Link>
        </div>
      </div>
    </header>
...
export function LandingHero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden bg-gradient-to-br from-white via-[#eef1fa] to-[#e8fafa]">
      {/* Background blobs — static when reduced motion is preferred */}
      <div
      ...
      <div
        ...
        style={
          prefersReduced
            ? undefined
            : { animation: "blob2 9s ease-in-out 4s infinite" }
        }
      />
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-14 items-center w-full">
        ...
        <div className="space-y-7">
          ...
          <motion.div
            ...
            className="md:hidden"
          >
            <Link href="/cadastro" className="block group" aria-label="Ver demonstração — Começar grátis">
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden group-hover:border-[#2DC6C6]/60 group-hover:shadow-cyan-100 transition-all duration-300">
                {/* Mockup header */}
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
                  <div className="flex items-center gap-2">
                    <Image
                      ...
                      priority
                    />
                    <span className="font-bold text-sm text-[#1B2A6B]">DrVet</span>
                  </div>
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {MOCK_STATS.map((s) => (
                      <div key={s.l} className={`rounded-xl p-2.5 ${s.c}`}>
                        <p className="text-[10px] opacity-70 leading-tight">{s.l}</p>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {MOCK_APPOINTMENTS.slice(0, 2).map((a) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span className="text-base">{a.sp}</span>
                            <div>
                              <p className="text-xs font-semibold">{a.pet}</p>
                              <p className="text-[10px] text-gray-400">{a.tu}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-xs font-semibold">{a.h}</p>
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${a.sc}`}>
                              {a.lb}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="bg-gradient-to-r from-[#1B2A6B] to-[#2DC6C6] rounded-xl py-2.5 text-white text-sm font-semibold text-center flex items-center justify-center gap-1.5 group-hover:shadow-md transition-all">
        ...
        >
          {/* Wrap in Link so all clicks convert — fixes 30.90% dead clicks */}
          <Link href="/cadastro" className="block group" aria-label="Ver demonstração — Começar grátis">
            <motion.div
              className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-6 space-y-4 ml-6 group-hover:border-[#2DC6C6]/50 group-hover:shadow-cyan-100 transition-colors duration-300"
            ...
            >
                ...
                <div className="space-y-2">
                  {MOCK_APPOINTMENTS.map((a) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-xs font-semibold">{a.h}</p>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${a.sc}`}>
                          {a.lb}
                        </span>
                      </div>
                    </div>
              ...
              <div className="h-0 overflow-hidden group-hover:h-auto group-hover:pt-0 transition-all duration-200">
                <div className="bg-gradient-to-r from-[#1B2A6B] to-[#2DC6C6] rounded-xl py-2.5 text-white text-sm font-semibold text-center flex items-center justify-center gap-1.5">
                  Começar grátis <ArrowRight className="w-3.5 h-3.5" />
                </div>
              </div>
            </motion.div>
          ...
          {/* Floating badges — pointer-events-none so they never cause dead clicks */}
          <motion.div
            className="absolute top-4 -right-6 bg-white shadow-xl rounded-2xl px-4 py-2.5 flex items-center gap-2 border border-gray-100 pointer-events-none select-none"
          ...
          <motion.div
            className="absolute bottom-4 -left-6 bg-white shadow-xl rounded-2xl px-4 py-2.5 flex items-center gap-2 border border-gray-100 pointer-events-none select-none"
            animate={prefersReduced ? {} : { y: [0, -8, 0] }}
