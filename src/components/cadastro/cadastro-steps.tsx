...
export function StepTipo({
  ...
  const options: {
  ...
  }[] = [
    {
      ...
      desc: "Estabelecimento com múltiplos veterinários, atendentes e equipe.",
    },
    {
      value: "hospital",
      icon: ClipboardList,
      title: "Hospital Veterinário",
      desc: "Hospital com internação, UTI e equipe ampla.",
    },
    {
      value: "autonomous",
      icon: Stethoscope,
      title: "Veterinário Autônomo",
      desc: "Trabalho solo ou consultório individual sem equipe fixa.",
    },
  ];
    ...
    <div className="space-y-4">
      <div className="mb-6">
        <h2 className="text-2xl font-extrabold text-gray-900">
          Como você vai usar o DrVet?
        </h2>
        <p className="text-gray-500 mt-1 text-sm">
          Isso nos ajuda a configurar sua conta do jeito certo.
        </p>
      </div>
      {errors.clinicType && (
        <p className="text-xs text-red-500">{errors.clinicType}</p>
      )}
      {options.map(({ value, icon: Icon, title, desc }) => (
        ...
        >
          <div
            className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${form.clinicType === value ? "bg-gradient-to-br from-[#1B2A6B] to-[#2DC6C6] text-white" : "bg-gray-100 text-gray-500"}`}
          >
            <Icon className="w-5 h-5" />
          </div>
          <div className="flex-1">
            <p
              className={`font-semibold text-sm ${form.clinicType === value ? "text-[#1B2A6B]" : "text-gray-800"}`}
            >
              {title}
            </p>
          ...
          <div
            className={`w-5 h-5 rounded-full border-2 shrink-0 mt-0.5 flex items-center justify-center ${form.clinicType === value ? "border-[#1B2A6B] bg-[#1B2A6B]" : "border-gray-300"}`}
          >
            {form.clinicType === value && (
...
export function StepDados({
  ...
  return (
    <div className="space-y-4">
      <div className="mb-6">
        <h2 className="text-2xl font-extrabold text-gray-900">
          {isAuto ? "Seus dados profissionais" : "Dados da sua clínica"}
        </h2>
        <p className="text-gray-500 mt-1 text-sm">
          {isAuto
            ? "Algumas informações básicas sobre você."
      ...
      <Field
        label={isAuto ? "Seu nome completo" : "Nome do responsável"}
  ...
  return (
    <div className="space-y-4">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Dados de login do {isAuto ? "seu usuário" : "administrador da conta"}.
        </p>
      </div>
      ...
      <Field label="Confirmar senha" error={errors.ownerConfirmPassword}>
        <PasswordInput
          ...
          onChange={(v) => setField("ownerConfirmPassword", v)}
          placeholder="Repita a senha"
        />
      </Field>
      <div className="bg-[#1B2A6B]/5 rounded-xl p-4 border border-[#2DC6C6]/30 text-sm">
        <p className="font-semibold text-[#1B2A6B] mb-1">
          {isAuto ? "Conta autônoma" : "Conta administrador"}
        </p>
        <p className="text-gray-500">
          {isAuto
            ? "Acesso completo a todos os módulos como veterinário autônomo."
            : "Como administrador você gerencia toda a equipe, módulos e configurações."}
        </p>
      </div>
    </div>
  );
...
export function StepEquipe({
  ...
  function removeUser(id: string) {
    setField(
      "additionalUsers",
      form.additionalUsers.filter((u) => u.id !== id),
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Cadastre os membros agora ou depois pelo painel.
        </p>
      </div>
      <p className="text-sm font-medium text-gray-700">
        Deseja cadastrar usuários da equipe agora?
      </p>
      {errors.hasAdditionalUsers && (
        <p className="text-xs text-red-500">{errors.hasAdditionalUsers}</p>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        {(
          ...
          ] as { val: boolean; label: string; Icon: React.ElementType }[]
        ).map(({ val, label, Icon }) => (
          <button
            key={String(val)}
            type="button"
            onClick={() => {
              setField("hasAdditionalUsers", val);
              if (!val) setField("additionalUsers", []);
            }}
        ...
        <div className="space-y-4 mt-2">
          {form.additionalUsers.map((u, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-[#1B2A6B] uppercase tracking-wider">
                  Usuário {i + 1}
                </span>
                <button
                  type="button"
              ...
              <Field label="Senha temporária" error={errors[`u${i}_pw`]}>
                <PasswordInput
                  value={u.password}
                  onChange={(v) => updateUser(u.id, "password", v)}
                />
              </Field>
            </div>
          ))}
          ...
          {form.additionalUsers.length === 0 && (
            <p className="text-xs text-gray-400 text-center">
              Clique acima para adicionar o primeiro usuário.
            </p>
          )}
        </div>
      )}
...
export function StepConfirmar({ form }: { form: RegFormData }) {
  const isAuto = form.clinicType === "autonomous";
  const typeLabel =
    form.clinicType === "clinic"
      ? "Clínica Veterinária"
      : form.clinicType === "hospital"
  ...
  return (
    <div className="space-y-5">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Confirme antes de criar sua conta.
        </p>
      </div>
      ...
      <div className="bg-gray-50 rounded-2xl p-5 border border-gray-200 space-y-3 text-sm">
        ...
        {!isAuto && (
          <SummaryRow label="Nome da clínica" value={form.clinicName} />
        )}
        ...
        {!isAuto && (
          <SummaryRow
            ...
            value={
              form.hasAdditionalUsers && form.additionalUsers.length > 0
        ...
        form.additionalUsers.length > 0 && (
          <div className="space-y-2">
            ...
            {form.additionalUsers.map((u) => (
              <div
                key={u.id}
                className="flex items-center gap-3 bg-white rounded-xl px-4 py-2.5 border border-gray-200"
              >
